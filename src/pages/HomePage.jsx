...
const HomePage = () => {
  ...
  const [filtersActive, setFiltersActive] = useState(false); // NEW

  const onSearch = async (newQuery) => {
    setQuery(newQuery);
    setFiltersActive(false); // Reset filters if searching
  ...
  const onFilter = ({ genre, year, rating, showFavorites }) => {
    ...
    setMovies(filtered);
    setFiltersActive(true); // Tell UI to use filtered list
  };
  ...
  return (
    <Container>
      ...
      {!query && !filtersActive && <TrendingMovies movies={trendingMovies} />}
